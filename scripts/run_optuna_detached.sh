...
#
# Usage:
#   ./scripts/run_optuna_detached.sh [trials] [workers]
#
...
SESSION="optuna-detached"
LOG="${LOG_DIR}/optuna_detached.log"
...
# Ensure at least one complete trial before starting detached workers, so a
...
tmux new-session -d -s "${SESSION}" \
    "cd ${ROOT} && uv run python hpc_replay.py optimize --trials ${TRIALS} --workers ${WORKERS} --resume 2>&1 | tee ${LOG}"
